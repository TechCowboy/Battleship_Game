...
{
	char apantisi;
	int winCounter=0,loseCounter=0,shotResult=0,myColnum=1,myrow=1,formation,row, col,A[10][10],j;
	int g=0,i;
	...
	for (row = 0; row < MAX_ROWS; row++)
	{
		...
		for (col = 0; col < MAX_COLS; col++)
		{
			...
			map[row][col] = pFormation[formation][row*MAX_ROWS+col];
			if (map[row][col]=='.')
			{
				map[row][col]=' ';
			}
